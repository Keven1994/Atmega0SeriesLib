...
#define ic(x) typename utils::integralConstant<mem_width,x>
                ...
                using ccl_lut0 = ic(0x10); using ccl_lut1 = ic(0x11); using ccl_lut2 = ic(0x12); using ccl_lut3 = ic(
                        0x13);
                using ac0_out = ic(0x20); using adc0_comp = ic(0x24);
                using usart0_xck0 = ic(0x60); using usart0_xck1 = ic(0x61); using usart1_xck0 = ic(
                        0x62); using usart1_xck1 = ic(0x63);
                using usart2_xck0 = ic(0x64); using usart2_xck1 = ic(0x65); using usart3_xck0 = ic(
                        0x66); using usart3_xck1 = ic(0x67);
...
#undef ic
            ...
            struct generatorChannel<0> {
                ...
                struct PortAGenerator {
                    static_assert(static_cast<mem_width>(pinNumber) < 8, "only pins [0,7] allowed");
                    static inline constexpr mem_width value = 0x40 + static_cast<mem_width>(pinNumber);
                };

                template<auto &pinNumber>
                struct PortBGenerator {
                    static_assert(static_cast<mem_width>(pinNumber) < 8, "only pins [0,7] allowed");
                    static inline constexpr mem_width value = 0x48 + static_cast<mem_width>(pinNumber);
                };
