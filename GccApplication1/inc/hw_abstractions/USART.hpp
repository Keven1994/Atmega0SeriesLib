...
namespace AVR::usart {
        ...
        using Basic_TransferMode =  typename mcu::USART::TransferMode;
        using TransferMode = Basic_TransferMode<>;

        template<typename mcu = DEFAULT_MCU>
        using Basic_Prescaler = typename mcu::USART::Prescaler;
        using Prescaler = Basic_Prescaler<>;

        template<typename mcu = DEFAULT_MCU>
        using Basic_SPI_Comp = typename mcu::SPI;
        using USART_Comp = Basic_USART_Comp<>;
        ...
        using USART = AVR::spi::details::USART<RW,accesstype, SPI_Comp::Component_t,typename instance::t1, typename instance::t2, SPI_Comp::template SPIMasterSetting<msb,clockDouble,slaveSelectDisable,tmode,buffered,waitForReceive,prescaler>, bit_width>;
